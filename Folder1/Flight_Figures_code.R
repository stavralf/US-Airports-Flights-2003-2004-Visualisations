...
ggplot()+
  ...
  scale_x_discrete(limits = c("January", "February", 'March',"April", "May", "June", "July", "August", 'September'
...
rate = (month_cancel$x/total_flights_per_month$x)*100
rate1 = (month_cancel1$x/total_flights_per_month1$x)*100
...
ggplot()+
  ...
  scale_x_discrete(limits = c("January", "February", 'March',"April", "May", "June", "July", "August", 'September'
                              , "October", "November", "December" ))+
  labs(y = 'Rate', title = "Percentage of Flight Cancellations per Month in USA Airports in 2003",
       colour = 'Year', x = 'Month')+
  theme(axis.title.x = element_text(size = 13, margin = margin(t = 15, r = 20, b = 0, l = 0)),
        axis.title.y = element_text(size = 13, margin = margin(t = 15, r = 20, b = 0, l = 0)),
        plot.title = element_text(size = 14, margin = margin(t = 45, r = 20, b = 20, l = 0)),
...
#Rates
cancel_very_long_rate = (cancel_distance[1,2]/sum(very_long))*100
cancel_long_rate = (cancel_distance[1,2]/sum(long))*100
cancel_medium_rate = (cancel_distance[1,2]/sum(medium))*100
cancel_short_rate = (cancel_distance[1,2]/sum(short))*100
...
x = c('Very Long', 'Long', 'Medium', 'Short')
...
ggplot()+
  ...
  labs(x = 'Distance Type', y = 'Rate')+
  labs(title = "Percentage of Cancelled flights within each Flight Distance type (%)")+
  theme(axis.title.x = element_text(size = 13, margin = margin(t = 15, r = 20, b = 0, l = 0)),
      axis.title.y = element_text(size = 13, margin = margin(t = 15, r = 20, b = 0, l = 0)),
      plot.title = element_text(size = 14, margin = margin(t = 45, r = 20, b = 20, l = 0)),
      legend.position = 'none')
...
samp_w_delay = sample(x = flights$WeatherDelay[flights$Origin == 'ATL'], size = 100000)# We take samples
samp_c_delay = sample(flights$CarrierDelay[flights$Origin == 'ATL'], size = 100000)
samp_secur_delay = sample(flights$SecurityDelay[flights$Origin == 'ATL'], size = 100000)
samp_NAS_delay = sample(flights$NASDelay[flights$Origin == 'ATL'], size = 100000)
samp_late_aircraft_delay = sample(flights$LateAircraftDelay[flights$Origin == 'ATL'], size = 100000)
...
ggplot(delays, aes(x = Type, fill = Type))+
  geom_bar(aes(x = reorder(Type, Type, function(x)-length(x)),y = (..count..)/sum(..count..)*100))+
...
samp_w_delay_t = sample(x = flights$WeatherDelay, size = 100000)# We take samples
samp_c_delay_t = sample(flights$CarrierDelay, size = 100000)
samp_secur_delay_t = sample(flights$SecurityDelay, size = 100000)
samp_NAS_delay_t = sample(flights$NASDelay, size = 100000)
samp_late_aircraft_delay_t = sample(flights$LateAircraftDelay, size = 100000)
...
ggplot(delays_t, aes(x = Type, fill = Type))+
  geom_bar(aes(x = reorder(Type, Type, function(x)-length(x)),y = 100*(..count..)/sum(..count..)))+
  labs(x = "Type of Delay", y = "Percentage", title = "Percentage of delayed flights based on Delay Type in USA Airports in 2003 ")+
  theme(axis.title.x = element_text(size = 13, margin = margin(t = 15, r = 20, b = 0, l = 0)),
        axis.title.y = element_text(size = 13, margin = margin(t = 15, r = 20, b = 0, l = 0)),
        plot.title = element_text(size = 14, face = 'bold', margin = margin(t = 45, r = 20, b = 20, l = 0)),
        legend.position = "none")
...
sample_dep_delay_day = flights[which(row.names(flights) %in% sample(row.names(flights), size = 100000,
                                                                    replace = FALSE)), 
                               c('DepDelay','DayOfWeek',"Month")]

sample_dep_delay_day_1 = flights1[which(row.names(flights1) %in% sample(row.names(flights1), size = 100000,
                                                                    replace = FALSE)), 
                               c('DepDelay','DayOfWeek',"Month")]
...
first_year =
  ggplot(days_delay)+
    ...
    labs(x = "", y = "Minutes", title = "Average delay in flights per day in US Airports in 2003 (%) ")+
    theme(axis.title.x = element_text(size = 13, margin = margin(t = 15, r = 20, b = 0, l = 0)),
          axis.title.y = element_text(size = 13, margin = margin(t = 15, r = 20, b = 0, l = 0)),
          plot.title = element_text(size = 15, face = 'bold', margin = margin(t = 45, r = 20, b = 20, l = 0)))
...
second_year = 
  ggplot(days_delay_1)+
    ...
    scale_x_discrete(limits = c("Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"))+
    labs(x = "", y = "", title = "Average delay in flights per day in US Airports in 2004 (%) ")+
    theme(axis.title.x = element_text(size = 13, margin = margin(t = 15, r = 20, b = 0, l = 0)),
          axis.title.y = element_text(size = 13, margin = margin(t = 15, r = 20, b = 0, l = 0)),
          plot.title = element_text(size = 15, face = 'bold', margin = margin(t = 45, r = 20, b = 20, l = 0)))
...
one = 
ggplot(df)+
  geom_col(aes(x = DayOfWeek, y = DepDelay, fill = Season), position = 'dodge')+
  scale_x_discrete(limits = c("Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"))+
  theme(axis.title.x = element_text(size = 13, margin = margin(t = 15, r = 20, b = 0, l = 0)),
        axis.text.x = element_text(angle = 90),
        axis.title.y = element_text(size = 12, margin = margin(t = 15, r = 20, b = 0, l = 0)),
        plot.title = element_text(size = 14, margin = margin(t = 45, r = 20, b = 20, l = 0), hjust = 0.5),
        legend.position = 'top',
        legend.title = element_blank())+
  labs (x= "",y = "Minutes",title = "2003")
                 
 


for (i in 1:nrow(sample_dep_delay_day_1)){
  if ((1<=sample_dep_delay_day_1$Month[i] && sample_dep_delay_day_1$Month[i]<=4) | 
      (9 <= sample_dep_delay_day_1$Month[i] && sample_dep_delay_day_1$Month[i] <= 12)){
    sample_dep_delay_day_1$Season[i] = 'Winter'
  }else{
    sample_dep_delay_day_1$Season[i] = "Summer"
  }
}


df_1 = aggregate(.~Season+DayOfWeek, sample_dep_delay_day_1, mean)# The average departure delay per day of the week and per Period(Summer/Winter)
...
title = text_grob('Average Flight Departure delay for each Day of the Week
             in Summer and Winter period', size = 14, face = 'bold')
